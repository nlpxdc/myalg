package structure.logic.keyvalue.set.variant.disjointset;
...
class IdxnoDisjointSetAppV2 {
    public static void main(String[] args) {
        System.out.println("aa");
    }
}

class IndxnoDisjointSetV2 implements DisjointSetAdt {

    int size;
    //下标idxno充当no序号，用负数表示根节点的高度
    int[] parentAry;
//    int[] heightAry;
//    int[] sizeAry;

    IndxnoDisjointSetV2(int size) {
        this.size = size;
...
//            return pAry[no] = noParentNo;
    ...
    void union2(int no1, int no2) {
        ...
        if (parentAry[root1] < parentAry[root2]) {
            //绝对值负数，root1高度大于root2高度，root2矮小，root2挂到root1下
            parentAry[root2] = root1;
        } else if (parentAry[root1] > parentAry[root2]) {
            //绝对值负数，root1高度小于root2，root2更高，root1挂到root2下
