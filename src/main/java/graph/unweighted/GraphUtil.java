...
//    public static int traverse(final int n, final BiConsumer<Integer, boolean[]> maxConnectedChildGraphTraverse) {
//        //初始化临时数组，记录访问状态
//        boolean[] visited = new boolean[n];
//
...
//    }
    ...
    public static AllVo traverse(final GraphMeta graphMeta, final GraphFunc<GraphMeta, SingleParam, GraphTempVo, SingleVo> maxConnectedChildGraphTraverse) {
        //初始化临时数组，记录访问状态
//        boolean[] visited = new boolean[n];
        GraphTempVo graphTempVo = new GraphTempVo(graphMeta);
//        SingleVo singleVo = new SingleVo(graphMeta.n);

        AllVo allVo = new AllVo();
        int maxConnectedChildGraphCnt = 0;
        for (int i = 0; i < graphMeta.n; i++) {
            Integer firstUnVisited = getFirstUnVisited(graphTempVo.visited);
            SingleParam singleParam = new SingleParam(firstUnVisited, maxConnectedChildGraphCnt);
            if (firstUnVisited != null) {
                maxConnectedChildGraphCnt++;
//                maxConnectedChildGraphTraverse.accept(firstUnVisited, visited);
                SingleVo singleVo = maxConnectedChildGraphTraverse.accept(graphMeta, singleParam, graphTempVo);
